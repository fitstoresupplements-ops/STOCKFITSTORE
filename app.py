...
# Búsqueda flexible de columnas para evitar el error de estructura
columnas_actuales = {col.lower(): col for col in df.columns}
col_id_real = next(
    (columnas_actuales[c] for c in columnas_actuales if c == "id"), None
)
col_nombre_real = next(
    (columnas_actuales[c] for c in columnas_actuales if "nombre" in c), None
)

if not df.empty and col_id_real and col_nombre_real:
    df.rename(
        columns={col_id_real: "ID", col_nombre_real: "Nombre"}, inplace=True
    )
    if "marca" in columnas_actuales:
        df.rename(columns={columnas_actuales["marca"]: "Marca"}, inplace=True)
    if "precio base" in columnas_actuales:
        df.rename(
            columns={columnas_actuales["precio base"]: "Precio Base"},
            inplace=True,
        )
    ...
    col_pres = next(
        (
            ...
            if "presentacion" in col.lower() or "presentación" in col.lower()
        ),
        None,
    )
    ...
    def construir_display(row):
        marca = (
            str(row.get("Marca", "")).strip() if "Marca" in df.columns else ""
        )
        nombre = str(row.get("Nombre", "")).strip()
        pres = str(row.get(col_pres, "")).strip() if col_pres else ""
        sabor = (
            str(row.get("Sabor", "")).strip() if "Sabor" in df.columns else ""
        )
    ...
    if pestana == "📦 Inventario General":
        ...
        if sucursal_sel == "San Javier":
            col_sj = next(
                (c for c in df_inventario.columns if "javier" in c.lower()),
                "San Javier",
            )
        ...
        with c2:
            unidades_totales = (
                int(df_inventario["Stock Total"].sum())
                if "Stock Total" in df_inventario.columns
                else 0
            )
        ...
        with c3:
            valor_inventario = 0
            if (
                "Stock Total" in df_inventario.columns
                and "Precio Base" in df_inventario.columns
            ):
                valor_inventario = (
                    df_inventario["Stock Total"] * df_inventario["Precio Base"]
                ).sum()
            st.metric(
                label="Valorización Estimada",
                value=f"${valor_inventario:,.2f}",
            )
        with c4:
            stock_critico = (
                len(
                    df_inventario[
                        df_inventario["Stock Total"]
                        <= df_inventario["Stock Minimo"]
                    ]
                )
                if "Stock Total" in df_inventario.columns
                and "Stock Minimo" in df_inventario.columns
                else 0
            )
    ...
    # -------------------------------------------------------------------------
    # 2. REGISTRAR VENTA (CORREGIDO CON ENVÍO ESTRICTO POR ID)
    # -------------------------------------------------------------------------
    elif pestana == "🛒 Registrar Venta":
        st.subheader("Registrar Venta y Descuento de Stock por ID")

        sucursal_venta = st.selectbox(
            "Punto de Venta",
        ...
        prod_seleccionado_display = st.selectbox(
            "Producto (Marca - Nombre - Presentación - Sabor)",
            productos_disponibles,
            key="venta_producto",
        )
        ...
        if sucursal_venta == "San Javier":
            col_suc = next(
                (c for c in columnas_disponibles if "javier" in c.lower()),
                "Stock Total",
            )
            ...
            col_suc = next(
                (c for c in columnas_disponibles if "alem" in c.lower()),
                "Stock Total",
            )
        ...
        if prod_seleccionado_display and not df.empty:
            ...
            if not fila_prod.empty:
                id_real_prod = (
                    str(fila_prod["ID"].values[0]).strip()
                    if "ID" in fila_prod.columns
                    else ""
                )
                nombre_real_prod = (
                    str(fila_prod["Nombre"].values[0])
                    if "Nombre" in fila_prod.columns
                    else ""
                )
                sabor_real_prod = (
                    str(fila_prod["Sabor"].values[0])
                    if "Sabor" in fila_prod.columns
                    else ""
                )

                if col_suc in fila_prod.columns:
                    stock_actual = int(
                        limpiar_numero(fila_prod[col_suc].values[0])
                    )
                if "Precio Base" in fila_prod.columns:
                    precio_base = float(
                        limpiar_numero(fila_prod["Precio Base"].values[0])
                    )
        ...
        if sucursal_venta in ["Alem", "San Javier"]:
            ...
            precio_sugerido = (
                precio_base / 0.9 if 0.9 > 0 else precio_base
            )

        st.info(
            f"🆔 ID del producto: **{id_real_prod}** | Stock en {sucursal_venta}: **{stock_actual}** | **Precio: ${precio_sugerido:,.2f}**"
        )
        ...
        with st.form("form_venta_local"):
            ...
            btn_registrar_venta = st.form_submit_button(
            ...
            if btn_registrar_venta:
                if not id_real_prod:
                    st.error(
                        "El producto seleccionado no cuenta con un ID válido."
                    )
                elif cant_venta > stock_actual:
                    st.error(
                        "No hay suficiente stock para realizar la venta en esta sucursal."
                    )
                else:
                    total_venta = cant_venta * precio_sugerido

                    # PAYLOAD ESTRICTO USANDO SOLO EL ID
                    payload = {
                        "accion": "descontar",
                        "id": id_real_prod,
                    ...
                    try:
                        ...
                        if res.status_code == 200:
                            ...
                            if res_json.get("status") == "success":
                                if (
                                    "ventas" not in st.session_state
                                    or not isinstance(
                                ...
                                st.cache_data.clear()
                                st.success(
                                    f"¡Venta registrada con éxito! Fila modificada en Sheets: {res_json.get('fila')}"
                                )
                            else:
                                st.error(
                                    f"Error desde el servidor: {res_json.get('message')}"
                                )
                        else:
                            st.error(
        ...
        if tipo_ingreso == "Producto Nuevo":
            ...
            with st.form("form_ingreso_nuevo"):
                ...
                with col_n1:
                    ...
                    categoria_nuevo = st.text_input(
                        "Categoría (ej. Creatina, Proteína)"
                    )
                with col_n2:
                    presentacion_nuevo = st.text_input(
                        "Presentación (ej. 300g, 60 caps)"
                    )
                    sabor_nuevo = st.text_input("Sabor")
                    precio_base_nuevo = st.number_input(
                        "Precio Base ($)", min_value=0.0, step=100.0
                    )

                col_n3, col_n4 = st.columns(2)
                with col_n3:
                    suc_ingreso_nuevo = st.selectbox(
                        "Sucursal de Destino",
                        ["Alem", "San Javier", "Hulk Gym"],
                        key="suc_nue",
                    )
                with col_n4:
                    cant_ingreso_nuevo = st.number_input(
                        "Cantidad a Ingresar", min_value=1, value=1, key="cant_nue"
                    )

                btn_guardar_nuevo = st.form_submit_button(
                    "Registrar Nuevo Producto y Stock"
                )
                    ...
                    if nombre_nuevo:
                        ...
                        try:
                            ...
                            if res.status_code == 200:
                                st.cache_data.clear()
                                st.success(
                                    f"¡Nuevo producto '{marca_nuevo} - {nombre_nuevo}' registrado con éxito!"
                                )
                                st.rerun()
                            else:
                                st.error(
                                    "Error al registrar el producto nuevo en la planilla."
                                )
                        except Exception as e:
                            st.error(f"Falla de conexión: {e}")
                    else:
            ...
            if prod_elegido and not df.empty:
                ...
                if not filas_prod.empty:
                    ...
                    categoria_existente = primera_fila.get(
                        "Categoría"
                        if "Categoría" in df.columns
                        else "Categoria",
                        "",
                    )
                    presentacion_existente = primera_fila.get(
                        "Presentacion"
                    ...
                    if "Precio Base" in primera_fila:
                        precio_auto = float(
                            limpiar_numero(primera_fila["Precio Base"])
                        )

                    if "Sabor" in df.columns:
                        sabores_existentes = [
                            str(s).strip()
                            for s in filas_prod["Sabor"].dropna().unique().tolist()
                            if str(s).strip()
            ...
            if sabores_existentes:
                tipo_sabor = st.radio(
                    "¿El sabor ya está registrado para este producto o es un sabor nuevo?",
                    ["Sabor Existente", "Sabor Nuevo"],
                    horizontal=True,
                    key="radio_tipo_sabor",
                )
            else:
                st.info(
                    "No hay sabores previos registrados. Se registrará como nuevo sabor."
                )
                tipo_sabor = "Sabor Nuevo"

            with st.form("form_ingreso_existente"):
                if tipo_sabor == "Sabor Existente" and sabores_existentes:
                    sabor_final = st.selectbox(
                        "Seleccionar Sabor Existente",
                        sabores_existentes,
                        key="select_sabor_existente",
                    )
                else:
                    sabor_final = st.text_input(
                        "Ingrese el Nombre del Nuevo Sabor",
                        key="input_nuevo_sabor",
                    )

                col_e1, col_e2 = st.columns(2)
                with col_e1:
                    suc_ingreso_ex = st.selectbox(
                        "Sucursal de Destino",
                        ["Alem", "San Javier", "Hulk Gym"],
                        key="suc_ex",
                    )
                with col_e2:
                    cant_ingreso_ex = st.number_input(
                        "Cantidad a Ingresar", min_value=1, value=1, key="cant_ex"
                    )

                btn_guardar_existente = st.form_submit_button(
                    "Sumar Stock / Registrar Variante"
                )
                ...
                if btn_guardar_existente:
                    if tipo_sabor == "Sabor Nuevo" and not sabor_final.strip():
                        ...
                        try:
                            ...
                            if res.status_code == 200:
                                st.cache_data.clear()
                                st.success(f"¡Ingreso registrado con éxito!")
                                st.rerun()
                            else:
                                st.error(
                                    "Error al registrar el ingreso en la planilla."
                                )
                        except Exception as e:
                            st.error(f"Falla de conexión: {e}")
        ...
        if not df.empty and "Producto_Display" in df.columns:
            ...
            with st.form("form_eliminar"):
                prod_a_borrar_display = st.selectbox(
                    "Seleccionar Producto (Marca - Nombre - Presentación - Sabor)",
                    productos_lista,
                )

                id_baja_real = ""
                fila_baja = df[df["Producto_Display"] == prod_a_borrar_display]
                if not fila_baja.empty and "ID" in fila_baja.columns:
                    id_baja_real = str(fila_baja["ID"].values[0]).strip()

                motivo_baja = st.selectbox(
                    "Motivo",
                    ["Venta Realizada", "Merma / Daño", "Ajuste de Inventario"],
                )
                ...
                if btn_eliminar:
                    ...
                    if res.status_code == 200:
                        st.cache_data.clear()
                        st.success(f"¡Stock actualizado correctamente por ID!")
                        st.rerun()
                    else:
    ...
    # -------------------------------------------------------------------------
        ...
        st.write(
            "Modifica los valores directamente sobre la tabla interactiva y guarda los cambios."
        )
    ...
    # -------------------------------------------------------------------------
        ...
        if (
            "ventas" not in st.session_state
            or st.session_state["ventas"].empty
        ):
            st.info(
                "Aún no hay ventas registradas en esta sesión para mostrar estadísticas."
            )
        else:
            ...
            periodo = st.selectbox(
                "Filtrar por período", ["Todos", "Mes Actual", "Año Actual"]
            )
            ...
            if not df_ventas.empty:
                ventas_por_pv = (
                    df_ventas.groupby("Punto de Venta")["Total"]
                    .sum()
                    .reset_index()
                )
                st.bar_chart(ventas_por_pv.set_index("Punto de Venta"))

                mejor_pv = ventas_por_pv.loc[
                    ventas_por_pv["Total"].idxmax()
                ]["Punto de Venta"]
                st.success(
                    f"🏆 El punto de venta con mayor salida es: **{mejor_pv}**"
                )
    ...
    st.info(
        "Revisá que la primera fila de tu Google Sheet mantenga los encabezados correctos."
    )         